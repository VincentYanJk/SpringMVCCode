...
public class ClassScanner {
	...
	/**
	 * parameter: basePackage Path
	 * return result:all of class (Map<String, Class<?>>)
	 *
	 * */
	public static Map<String, Class<?>> scannerClass(String basePackage) {

		log.debug("Start to pulling all of class under the base package.");
		...
		try {
			...
			log.debug("End to pulling all of class under the base package.");
		...
		for (int i = 0; files != null && i < files.length; i++) {
			...
			if (file.isDirectory()) {
				...
				if (path.endsWith(".class")) {
					...
					String className = rootP.substring(0, rootP.lastIndexOf("/"))
							+ path.substring(path.lastIndexOf("/"), path.indexOf(".class"));
